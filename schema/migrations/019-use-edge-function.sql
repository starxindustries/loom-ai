-- ============================================================================
-- Update Task Execution to Use Supabase Edge Function
-- ============================================================================
...
BEGIN
  ...
  BEGIN
    ...
    -- Format: https://[project-ref].supabase.co/functions/v1/execute-task
    edge_function_url := COALESCE(
      current_setting('app.supabase_functions_url', true),
      'https://thyqmgekkwwlqcwkgsmb.supabase.co/functions/v1/execute-task'
    );
    ...
    SELECT * INTO api_response FROM http((
      ...
      ARRAY[
        ...
        http_header('Authorization', 'Bearer ' || COALESCE(
          ...
          'eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6InRoeXFtZ2Vra3d3bHFjd2tnc21iIiwicm9sZSI6ImFub24iLCJpYXQiOjE3MjcxNzE4NzIsImV4cCI6MjA0Mjc0Nzg3Mn0.Wd8Nh-VDXG8VwLMJNjdGOJQJzOJZJZJZJZJZJZJZJZJZ'
        ))
      ],
      'application/json',
      api_payload::text
    ...
    ELSE
      ...
      exec_error_message := COALESCE(
        ...
        'Edge Function call failed with status ' || api_response.status
      );
...
-- To customize the Edge Function URL, run:
-- ALTER DATABASE postgres SET app.supabase_functions_url = 'https://your-project-ref.supabase.co/functions/v1/execute-task';

COMMENT ON FUNCTION public.execute_scheduled_task(UUID) IS 'Executes scheduled tasks by calling Supabase Edge Function for real action execution';
